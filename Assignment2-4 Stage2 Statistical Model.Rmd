---
title: "Assignment2-4 Stage2 Statistical Model"
author: "Youran Zhou"
date: "2020/10/4"
...
predict_result = round(predict(binreg_model2, newdata=train, type="response")-0.40)

accuracy = (1- (sum(abs(predict_result - train$Tip))/nrow(train)))*100

predict_result = round(predict(binreg_model2, newdata=test, type="response")-0.40)

accuracy = (1- (sum(abs(predict_result - test$Tip))/nrow(test)))*100
...
training <- data.frame(cbind(actuals=ptrain$Freq , predicteds=(trainPred),(actuals=ptrain$Freq-(trainPred))^2))
...
test <- data.frame(cbind(actuals=ptest$Freq , predicteds=(trainPred),(actuals=ptest$Freq-(trainPred))^2))

mse <- sqrt(mean(test[,3]))
mse
